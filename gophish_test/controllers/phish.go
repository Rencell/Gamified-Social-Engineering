...
func (ps *PhishingServer) PhishHandler(w http.ResponseWriter, r *http.Request) {
	...
	if preview, ok := ctx.Get(r, "result").(models.EmailRequest); ok {
		...
		ptx, err = models.NewPhishingTemplateContextSms(&preview, preview.BaseRecipient, preview.RId)
		if err != nil {
			log.Error(err)
			http.NotFound(w, r)
			return
	...
	ptx, err = models.NewPhishingTemplateContextSms(&c, rs.BaseRecipient, rs.RId)
	if err != nil {
