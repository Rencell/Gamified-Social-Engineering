...
import type { GoPhish, GoPhishTotalScore, GoPhishTotalScoreSms } from '@/services/simulationService';

const openGophish = () => {
    const url = import.meta.env.VITE_GOPHISH_URL || 'https://gophish.example.com';
    window.open(url + '/sms_campaigns', '_blank');
};

const phishingData = ref<GoPhishTotalScoreSms>();
const campaignsCount = ref<number>(0);
const usersCount = ref<number>(0);

onMounted(async () => {
  try {
    const response = await SimulationService.get_total_score_sms();
    const respons2 = await SimulationService.get_all_sms();
    ...
    <section class="mx-auto max-w-7xl py-8 ">
        ...
        <div class="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card class="bg-card">
                ...
                <CardContent>
                    <div class="text-2xl font-bold text-foreground">{{ phishingData?.max_sms_sent ?? 0 }}</div>
