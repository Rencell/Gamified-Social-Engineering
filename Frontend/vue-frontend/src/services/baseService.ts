...
const baseService = {
    ...
    post: <T>(url: string, data: T): Promise<T> => session.post(url, data).then(res => res.data),
    put: <T>(url: string, data: T): Promise<T> => session.put(url, data).then(res => res.data),
...
export function serviceFactory<T>(basePath: string) {
  return {
    ...
    create: (data: T): Promise<T> => baseService.post<T>(basePath, data),
    update: (id: ID, data: T): Promise<T> => baseService.put<T>(`${basePath}${id}/`, data),
