...
<template>
    ...
    <Card class="bg-background">
        ...
        <CardContent class="space-y-4">
            ...
            <!-- <Email :from="quizData.from" :subject="quizData.subject" :body="quizData.body" :date="quizData.date" :link="quizData.link" /> -->
            <Email 
                v-if="!edittable"
                :from="quizData.email.from"
                :subject="quizData.email.subject"
                :date="quizData.email.date"
                :link="quizData.email.link"
                :body="quizData.email.body"
                :footer="quizData.email.footer"
            />
            <EmailEdittable
                v-else
                v-model:from="quizData.email.from"
                v-model:subject="quizData.email.subject"
                v-model:date="quizData.email.date"
                v-model:link="quizData.email.link"
                v-model:body="quizData.email.body"
                v-model:footer="quizData.email.footer"
                @addBody="quizData.email.body.push('New Content')"
                @remove-body="(index) => quizData.email.body.splice(index, 1)"
            />
        </CardContent>
