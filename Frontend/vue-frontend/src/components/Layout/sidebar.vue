...
<template>
          ...
          <SidebarGroupContent>
            <SidebarMenu class="space-y-3">
              <SidebarMenuItem v-for="item in navigationData.learning" :key="item.title">
                ...
                <p v-if="item.title === 'Assessments'"
                  class="text-xs font-medium text-slate-400 uppercase tracking-wider mt-3">Simulation</p>
              </SidebarMenuItem>
            </SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>
  ...
  <div
    class="scroll-smooth overflow-x-auto bg-secondary/50 backdrop-blur-lg absolute bottom-0 w-full p-4 z-20 text-white text-center md:hidden flex justify-between scrollbar-hide">
    <div class="flex flex-nowrap space-x-6">
      <div v-for="item in navigationData.learning" :key="item.title">
        <RouterLink :to="Array.isArray(item.url) ? item.url[0] : item.url" class="flex items-center" :class="{
      ...
      <div>
        <RouterLink :to="'/settings'" class="flex items-center" :class="{
